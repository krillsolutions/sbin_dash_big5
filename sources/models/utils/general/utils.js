...
import {gconfig, getFilter, getPeriod, getToken} from 'models/utils/general/boot';
...
export function showProcess(menu, cmp) {
	...
	if(typeof cmp == 'undefined') {
		...
		if(typeof $$(components[menu][cmp]) != 'undefined') {
...
export function setLabels() {
	...
	let ind = (getScreenType() == 'mobile' || getScreenType() == 'mobile_rotated')?3:8;
	for(let elm of getAllFilters()) {
		...
		if($$('loginbar'))
		if (typeof getFilter()[elm] != 'undefined') $$('loginbar').addView(new FilterIconView(app, "", elm), ind);
	}
	if($$('period')) {
	$$('period').define('label', "<span style='color:#fff;font-size:14px' class='mdi mdi-calendar-check' > Période : "+getPeriod()['d1']+' - '+getPeriod()['d2'] +"</span>");
	$$('period').refresh();
	}
}
...
export function setScreenType() {
	   ...
	   if( /Android|webOS|iPhone|iPad|Mac|Macintosh|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) && window.screen.width <=600) type = 'mobile';
	   if( /Android|webOS|iPhone|iPad|Mac|Macintosh|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) && window.screen.height <=600) type = 'mobile_rotated';
...
export function getTreeHierachy(data,levels=[], name = "name", values = ["value"]) {
