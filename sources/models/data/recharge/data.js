...
const rec_stat_tot = new webix.DataCollection({
    id : 'rec_stat_tot',
    url :  function(params) { return ServerController.getData('recharge', 'getStat', 'mnt_tot')}
});

const rec_stat_type = new webix.DataCollection({
    id : 'rec_stat_type',
    url :  function(params) { return ServerController.getData('recharge', 'getStat', 'mnt_rec_type')}
});


const rec_type_split = new webix.DataCollection({
    id : 'rec_type_split',
    url :  function(params) { return ServerController.getData('recharge', 'getRechargeType', 'vue1')}
});


const rec_type_trend = new webix.DataCollection({
    id : 'rec_type_trend',
    url :  function(params) { return ServerController.getData('recharge', 'getRechargeTrend', 'vue1')}
});

const rec_by_channel_split = new webix.DataCollection({
        id : 'rec_by_channel_split',
        url : function() {return ServerController.getData('recharge','getRechargeChannel','split')}
})

const rec_voucher_value_split = new webix.DataCollection({
        id : 'rec_voucher_value_split',
        url :  function(params) { return ServerController.getData('recharge', 'getRechargeValueSplit', 'vue1')}
        });

const rec_by_prod_by_type = new webix.DataCollection({
        id : 'rec_by_prod_by_type',
        url :  function(params) { return ServerController.getData('recharge', 'getRechargeSplit', 'vue1')}
        }); 
        
const rec_by_prod_by_type_exp = new webix.DataCollection({
        id : 'rec_by_prod_by_type_exp',
        url :  function(params) { return ServerController.getData('recharge', 'getRechargeSplit', 'vue2')}
                });          
...
export function getAllRecData() {
        ...
        return [ {type : "vue1", data : rec_type_split,func : 'getRechargeType'},
                 ...
                 {type : "vue1", data : rec_voucher_value_split, func : 'getRechargeValueSplit'},
...
export const functions_ref = {
        'stat_tot' : 'getStat', 'stat_type' : 'getStat', 'type_split' : 'getRechargeType', 'type_trend' : 'getRechargeTrend', 'sc_value_split' : 'getRechargeScratchValuesSplit',
        'geo_split' : 'getRechargeGeoSplit', 'ec_trend' : 'getECTrend', 'remb_trend' : 'getECRemb'
}   

export function getFiterDate(func, params, date){

                return ServerController.getDrillData('recharge',functions_ref[func], params, date);
