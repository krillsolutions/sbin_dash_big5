	   ...
	   grid : [
        ...
        init(view) {
			...
			$$("rev:vue1:trend")._isDataLoaded = 1;
                ...
                $('button.ui.button.revParType_'+periodSplit[1]).on('click',function(){
                    if( $$('rev:vue1:trend')._filter_level != 'm')
                            RevByTypeTrendView.filterHandler({data : {month : $$('rev:vue1:trend')._current_month }})
        
                })

                $('button.ui.button.revParType_'+periodSplit[0]).on('click',function(){
                    if ($$('rev:vue1:trend')._filter_level == 'm') {
                                    $$('rev:vue1:trend')._filter_level = 'a';
                ...
                getRevChartData('revType_trend').attachEvent("onBeforeLoad", function () {
        ...
        static filterHandler(params){
            ...
            $("button.ui.button.revParType_"+periodSplit[1]).addClass('active').siblings().removeClass('active')

            if(!$$('rev:vue1:trend')._months) $$('rev:vue1:trend')._months = [];
            if(!$$('rev:vue1:trend')._months.some(d => d == params.data.month)){
                    let dat = getFiterDate('split_trend', 'dt_trend', params.data.month+'-01', 'vue0');
                    $$('rev:vue1:trend').showProgress();
                    $$('rev:vue1:trend').disable()                        
                    dat.then( (d) => {
