...
import {setLabels, setScreenType, getScreenType} from "models/utils/general/utils";
...
export default class TopView extends JetView{
	config(){
		...
		return refData.waitData.then(() => {
			let ui = {
				rows:[
					( (getScreenType() == 'mobile' || getScreenType() == 'mobile_rotated')? MobileHeadToolBar : HeadBarView ),//HeadBarView,
					{ cols:[
					...
					{template:"<p class='title'></b></p>", height:20, css:"footer"}
				]
			};
			return ui;
		});
	...
	init(view){
		...
		webix.protoUI({
			...
			if(getScreenType() != 'mobile' && getScreenType() != 'mobile_rotated'){
		            $$('dashtitle').define('label',"<span style='color:#fff;'"+((getScreenType() == 'mobile')?'font-size:10px':'')+" >"+((getScreenType() == 'mobile')? '' : refs.dashTitle)+"</span>"  );
		            $$('dashtitle').refresh();
			}
		       setLabels(); 
		      
	      })
		gconfig["mobileMenuhide"] = (webix.storage.session.get("mobileMenuhide") == null) ? false : webix.storage.session.get("mobileMenuhide");
		webix.event(window, "resize",function(){
			
			if(getScreenType() != webix.storage.session.get('screenType')) setScreenType();
		
		})

		webix.DataStore.prototype.sorting.as.abs_sort = 
			function(a,b){ return Math.abs(a) > Math.abs(b) ? 1 : -1 }
		//webix.Touch.disable();
