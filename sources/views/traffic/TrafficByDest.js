...
import {components, color_ref,dash_titles} from "models/referential/genReferentials";
...
//                let _Type = ['onnet', 'offnet', 'international'];
                ...
                let days = dat.map(d => d.upd_dt).filter((v,i,arr) => arr.indexOf(v) == i)
                dat.forEach(elm => {
                ...
                _Type.forEach(elm => {
                    if(dat[0]._type == 'dt_trend'){
                        dataset.push({dimensions : ['upd_dt', 'traffic'], source : dat.filter(d => (d.traff_type == kp) && d.traff_dir == 'in'&& d.traff_cat == elm), isAdded : true});
                        series.push({type : 'bar',stack : elm+'_split', itemStyle : {color : function(para) {return color_ref.traffic[kp+'_in']}},
                        id :'traff_split_dest:in:'+elm+":"+kp, datasetIndex : dataset.length-1,barMinHeight: 10, //tooltip : {trigger : 'item'} ,
                        _isStack :true, name : elm  , encode : {x : 'upd_dt', y : 'traffic'} , 
                        }
                        );

                        dataset.push({dimensions : ['upd_dt', 'traffic'], source : dat.filter(d => (d.traff_type == kp) && d.traff_dir == 'out' && d.traff_cat == elm), isAdded : true});
                ...
                if(dat[0]._type == 'dt_trend') conf.legend['selectedMode'] = 'single';
			...
			title : [
				...
				{
					text : dash_titles['traffic'].traff_by_dest? dash_titles['traffic'].traff_by_dest : ""  ,
					//subtext : 'Revenu',
					left : '50%',
                    textAlign: 'center',
                    textStyle : {
        ...
        init(view) {
            ...
            let kp = this._kpi;
                ...
                components['traffic'].push( {cmp : "traff:"+kp+":vue1:dest", data :getTraffChartData('traffic_dest').config.id });
			...
			$$("traff:"+kp+":vue1:dest")._isDataLoaded =1;
                ...
                $$("traff:"+kp+":vue1:dest").data.attachEvent("onStoreLoad", function () {
