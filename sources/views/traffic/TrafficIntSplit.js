...
export default class TraffInTView extends JetView {
    ...
    var conf = color_ref.then((color_ref) => {
      return {
        ...
        options: {
          ...
          title: [
            {
              text: dash_titles["traffic"].top_int_traffic
                ? dash_titles["traffic"].top_int_traffic
                : "",
              left: "50%",
              top: 5,
              textAlign: "center",
          ...
          series: [
            {
              ...
              tooltip: {
                ...
                formatter: function (params) {
                  return (
                    params.name + ": " + formatter(params.value.value, "t")
                  );
                },
              },
              itemStyle: {
                color: function (p) {
                  if (
                    p.dataIndex < colors.length &&
                    p.value.country != "Autres" &&
                    p.value.country != "Inconnu"
                  )
              ...
              label: {
                ...
                formatter: function (params) {
                  return (
                    kFormatter(params.data.value) +
                    " (" +
                    d3.format(".2%")(params.data.value / params.data.tot) +
                    ")"
                  );
                },
  ...
  init(view) {
    ...
    components["traffic"].push({
      cmp: "traff:int:" + kp,
      data: getTraffChartData("int_traffic_sample").config.id,
    });
