...
export default class TraffByOfferView extends JetView {
  ...
  config() {
    ...
    return {
      ...
      beforedisplay: function (dat, conf) {
        if (dat.length != 0)
          conf.series[0].data = dat.filter((d) => d.traff_type == kp);
      },
      options: {
        tooltip: {
          formatter: function (params) {
            return (
              params.marker + params.name + " : " + formatter(params.value)
            );
          },
        ...
        title: [
          {
            //		top : 5,
            text: dash_titles["traffic"].by_bill_by_prod_by_offer
              ? dash_titles["traffic"].by_bill_by_prod_by_offer
              : "",
            //subtext : 'Revenu',
            left: "50%",
            textAlign: "center",
        ...
        label: {
          ...
          formatter: function (params) {
            return (
              ...
              " : " +
              (kp == "data"
                ? DatakFormatter(params.value)
                : kFormatter(params.value))
            );
          },
        },
        grid: [
  ...
  init(view) {
    ...
    components["traffic"].push({
      cmp: "traff:vue1:" + kp + ":offer",
      data: getTraffChartData("traffic_offer").config.id,
    });
    components["traffic"].push({
      cmp: "traff:vue1:export:offer",
      data: getTraffChartData("traffic_offer_exp").config.id,
    });
    getTraffChartData("traffic_offer").waitData.then((d) => {
      $$("traff:vue1:" + kp + ":offer")._isDataLoaded = 1;
      $$("traff:vue1:" + kp + ":offer").parse(
        getTraffChartData("traffic_offer")
      );
      $$("traff:vue1:" + kp + ":offer").enable();
      $$("traff:vue1:" + kp + ":offer").hideProgress();
    });

    $$("traff:vue1:" + kp + ":offer").data.attachEvent(
      "onStoreLoad",
      function () {
