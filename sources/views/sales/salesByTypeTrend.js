...
export default class SalesByTypeTrendView extends JetView{
	...
	config() {
	    ...
	    //minHeight : 200,
            beforedisplay : function(dat, conf, echart_obj) {
                ...
                let revs = dat.map(d => d.cat).filter((d,i,ar) => ar.indexOf(d) == i);
                ...
                revs.forEach(elm => {
                    if( dat.some(d => d._type == 'month_trend')  ){
                        dataset.push({dimensions : ['month', 'montant'], source : dat.filter(d => (d.cat == elm) && d._type == 'month_trend'), isAdded : true});
                        series.push({type : 'bar', itemStyle : {color : color_ref.revenue[elm]  /*function(para) {return color_ref.revenue[para.name]}*/},stack : 'split', id :'sales_split_trend:'+elm, datasetIndex : dataset.length,barMinHeight: 10, //tooltip : {trigger : 'item'} ,
                    ...
                    if(dat.some(d => d._type == 'dt_trend')) {
                        dataset.push({dimensions : ['upd_dt', 'montant'], source : dat.filter(d => (d.cat == elm) && d._type == 'dt_trend'), isAdded : true});
                    ...
                    echart_obj.on('click', {seriesId : 'sales_split_trend:'+elm,  seriesType : 'bar'}, (params) => { 
			    ...
			    SalesByTypeTrendView.filterHandler(params)
		    });
	   ...
	   grid : [
        ...
        init(view) {
                ...
                $$("sales:vue1:trend").showProgress();
                $$("sales:vue1:trend").disable();
                $$('sales:vue1:trend')._filter_level = 'a';
                $$('sales:vue1:trend')._months = [];
                $$('sales:vue1:trend')._days = [];
                $$('sales:vue1:trend')._current_month = getDates()['d1'].substr(0,7);
                $$('sales:vue1:trend')._current_upd_dt = getDates()['d1'];
                components['sales'].push( {cmp : "sales:vue1:trend", data :getSalesChartData('saleType_trend').config.id });
                getSalesChartData('saleType_trend').waitData.then((d) => {
			$$("sales:vue1:trend")._isDataLoaded = 1;
                        $$("sales:vue1:trend").parse(getSalesChartData('saleType_trend'));
                        $$("sales:vue1:trend").enable();
                        $$("sales:vue1:trend").hideProgress();

                });

                $('button.ui.button.salesParType_'+periodSplit[1]).on('click',function(){
                    if( $$('sales:vue1:trend')._filter_level != 'm')
                            SalesByTypeTrendView.filterHandler({data : {month : $$('sales:vue1:trend')._current_month }})
        
                })

                $('button.ui.button.salesParType_'+periodSplit[0]).on('click',function(){
                    if ($$('sales:vue1:trend')._filter_level == 'm') {
                                    $$('sales:vue1:trend')._filter_level = 'a';
                                    $$('sales:vue1:trend').data.filter((d)=> d._type == 'month_trend') ;

                    }
                }
                )                
                $$("sales:vue1:trend").data.attachEvent("onStoreLoad", function () {
                                updateChartReady("sales:vue1:trend");
                });
                getSalesChartData('saleType_trend').attachEvent("onBeforeLoad", function () {
        ...
        static filterHandler(params){
            ...
            $("button.ui.button.salesParType_"+periodSplit[1]).addClass('active').siblings().removeClass('active')

            if(!$$('sales:vue1:trend')._months) $$('sales:vue1:trend')._months = [];
            if(!$$('sales:vue1:trend')._months.some(d => d == params.data.month)){
                    let dat = getFiterDate('split_trend', 'dt_trend', params.data.month+'-01', 'vue2');
                    $$('sales:vue1:trend').showProgress();
                    $$('sales:vue1:trend').disable()                        
                    dat.then( (d) => {
                        $$('sales:vue1:trend')._months.push(params.data.month);
                        $$('sales:vue1:trend')._filter_level = 'm';
                        $$('sales:vue1:trend').parse(dat).then(()=>{$$('sales:vue1:trend').data.filter((d) =>  {
                            return (d._type != 'month_trend' && (d.period == params.data.month )) || d._type == 'rev_split';
                            });});
                        
                    $$('sales:vue1:trend')._current_month = params.data.month;
                    $$('sales:vue1:trend').enable();
                    $$('sales:vue1:trend').hideProgress();
            })}
            else {
                $$('sales:vue1:trend')._filter_level = 'm';
                $$('sales:vue1:trend')._current_month = params.data.month;
                $$('sales:vue1:trend').data.filter((d) =>  {
                    return (d._type != 'month_trend' && (d.period == params.data.month)) || d._type == 'rev_split';
                })
            }
