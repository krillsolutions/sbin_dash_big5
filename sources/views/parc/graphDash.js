    ...
    switch (getScreenType()) {
      case "mobile":
        ...
        cells.push({
          ...
          dy: 5,
          resize: true,
          header: new GraphHeadView(this.app, "", "offer_type", "homelines"),
          body: {
            type: "clean",
            margin: 0,
            rows: [
              {
                type: "clean",
                margin: 0,
                cols: [ParcByTypeView, ParcProdTypeView],
              },
              new PeriodSelector(this.app, "", "parcByType_prod", 2),
              ParcByTypeTrendView,
            ],
          },
        ...
        cells.push({
          ...
          dy: 5,
          resize: true,
          header: new GraphHeadView(this.app, "", "parc_other", "homelines"),
          body: {
            type: "clean",
            margin: 0,
            rows: [ParcStatusNetView, ParcNetAddView],
          },
          css: { "background-color": "#fff" },
        });
      ...
      case "mobile_rotated":
        ...
        cells.push({
          ...
          body: {
            type: "clean",
            margin: 0,
            cols: [ParcStatusNetView, ParcNetAddView],
          },
          css: { "background-color": "#fff" },
        });
    ...
    webix.event(window, "resize", function () {
      if ($$("top:menu").getSelectedId() == "parc") {
        if (getScreenTypeByMenu("parc") != getScreenType()) {
          setScreenTypeByMenu("parc", getScreenType());
          if (gconfig["dashboards"]["parc"]._container != null)
            gconfig["dashboards"]["parc"].refresh();
        }
      }
    });
  }
