...
import {components,dataDesc,color_ref,trafficYaxisType} from "models/referential/genReferentials";
...
export default class HomeTraffTrendView extends JetView{
	...
	config() {
        ...
        let series = [], dataset = [], yAxis = [],xAxis = []
        ...
        for (const elm of traffType) {
            ...
            xAxis.push({
                type: 'category',splitNumber  : 6,position : 'bottom',_type : elm,
                show : (yAxisIn == 0),
			    splitLine: {
			            show: false
                },
                axisPointer: {
                		...
                		axisTick: {show: (yAxisIn == 0), alignWithLabel: (yAxisIn == 0)},

    			})

            series.push({
                type : 'line',gridIndex : 1, encode : {x : 'month', y : 'traffic'},seriesLayoutBy: 'row',datasetIndex : dataset.length - 3,_kpi : elm,name : elm,yAxisIndex : yAxisIn,xAxisIndex : yAxisIn,
            ...
            },
            {
                ...
                itemStyle : {color : ((elm == 'voice')? 'red' : ((elm == 'sms')? 'green' : 'blue'))},yAxisIndex : yAxisIn, xAxisIndex :yAxisIn
            },
            {
		...
		return {
            ...
            beforedisplay : function(dat, conf, echart_obj){
                ...
                let xAxis = conf.xAxis
                echart_obj.on('legendselectchanged',function(ev){
                    echart_obj.setOption({
                        title : {text : trafficYaxisType[ev.name].name},
                        xAxis : xAxis.map(d => {
                            let t = (d._type == ev.name)
                            //console.log(d)
                            d.show = t
                            d.axisPointer = {show : t}
                            d.axisTick =  {show: t, alignWithLabel: t}
                           //console.log(d)
                            return d
                            //if(d._type != ev.name) {d.show = false; d.axisPointer = false}; return d })
                    })
                })})
        ...
        init(view) {
			...
			$$('home:vue1:trend:traff')._isDataLoaded = 1;
                ...
                getHomeChartData('traffic_split').data.attachEvent("onClearAll", function () {
