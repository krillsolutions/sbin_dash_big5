...
export default class HomeParcNewTrendView extends JetView{
	...
	config() {
		...
		return {
           ...
           minHeight : 80,
            charts_event : {
                ...
                'click' : [  {seriesId : 'home:parc_split', seriesType : 'bar'}, function(params){
		    ...
		    let tp =  (getScreenType() != 'mobile' && getScreenType() != 'mobile_rotated') ? true : false;
                    ...
                    $("button.ui.button.parcHome_"+periodSplit[1]).addClass('active').siblings().removeClass('active')
    
                    if(!$$('home:vue1:trend:parc')._months) $$('home:vue1:trend:parc')._months = [];
                    if(!$$('home:vue1:trend:parc')._months.some(d => d == params.data.month)){
                            let dat = getFiterDate('parc', 'dt_trend', params.data.month+'-01');
                            ...
                            dat.then( (d) => {
                                ...
                                $$('home:vue1:trend:parc').parse(dat).then(()=>{$$('home:vue1:trend:parc').data.filter((d) =>  {
                                    return d._type != 'month_trend' && (d.period == params.data.month || d._type == 'op_split' || d._type == 'bill_split')
                                    });});
			...
			options :{
			 tooltip : {trigger : 'axis',show : true,  axisPointer : { show : true} ,
			...
			yAxis: [
			    {
                    type: 'value',isDim : true,_type : 'trend',_dim : 'value',splitNumber : 4,
                    axisLabel: {
			...
			series : [
                {
                    ...
                    _type : 'month_trend', id : 'home:parc_split',
                    areaStyle: {
                ...
                {
                    type : 'line', encode : {x : 'upd_dt', y : 'trend_dt'}, _type : 'dt_trend',datasetIndex : 1,barMaxWidth : 20,barMinHeight: 10,itemStyle : {color : "blue"}
                }
			]
		}
		}
        ...
        init(view) {
                ...
                $$("home:vue1:trend:parc").showProgress();
                $$("home:vue1:trend:parc").disable();
                components['home'].push({cmp : "home:vue1:trend:parc", data : getHomeChartData('parc_trend').config.id});
            ...
            $('button.ui.button.parcHome_'+periodSplit[1]).on('click',function(){
                if( view._filter_level == 'a'){
                    ...
                    if(!view._months.some(d => d == view._current_month)){
                        let dat = getFiterDate('parc', 'dt_trend', view._current_month+'-01');
                        view.showProgress();
                        view.disable()
                        dat.then( (d) => {
                            view._months.push(view._current_month);
                            view.parse(dat).then(()=>{view.data.filter((d) =>  {
                                return d._type != 'month_trend' && (d.period == view._current_month || d._type == 'op_split' || d._type == 'bill_split')
                            });});
