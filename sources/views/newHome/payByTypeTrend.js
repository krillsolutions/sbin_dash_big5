...
export default class PayByTypeTrendView extends JetView{
	...
	config() {
                ...
                beforedisplay : function(dat, conf, echart_obj) {
                    ...
                    types.forEach(elm => {
                        if( dat.some(d => d._type == 'month_trend')  ){
                            dataset.push({dimensions : ['month', 'revenue'], source : dat.filter(d => (d.clt_type == elm) && (d._type == 'month_trend') ), isAdded : true});
                        ...
                        if(dat.some(d => d._type == 'dt_trend')) {
                            dataset.push({dimensions : ['upd_dt', 'revenue'], source : dat.filter(d => (d.clt_type == elm) && d._type == 'dt_trend' ), isAdded : true});
        ...
        init(view) {
            ...
            getHomeChartData('pay_trend').data.attachEvent("onClearAll", function () {
