...
export class BarUserView extends JetView {
  ...
  config() {
    ...
    if (config["type"] == "iconTop")
      config["icon"] = ressource_ref[this._src]["ress"];
    else config[config["type"]] = ressource_ref[this._src]["ress"];

    if (typeof ressource_ref[this._src]["label"] == "undefined")
      config["label"] = name;
...
export class BarLogoView extends JetView {
  ...
  config() {
    ...
    let style =
      "style='float:" +
      ressource_ref[src]["position"] +
      ";margin:0px;width:" +
      ressource_ref[src]["width"] +
      "px;height:" +
      ressource_ref[src]["height"] +
      "px'";
    var config = {
      template:
        "<img src='" + ressource_ref[this._src]["ress"] + "'" + style + ">",
      borderless: true,
