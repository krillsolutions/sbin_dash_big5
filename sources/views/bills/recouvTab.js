...
export default class RecouvTableView extends JetView{
	...
	config ()
	{
        ...
        let mths = getMonthsFromDate(getDates()["d2"], recouvr_months_offset).sort(),
            cols = [{
                id : "month", 
                header : { height : 20, text : "Mois d'observation" ,css : {"text-align":"center"}},adjust : true,css : { "background-color":"#ffffdd", "text-align":"center","font-family" : "Roboto, sans-serif", "font-size" : "12px"  },
            }],
        ...
        mths.forEach((m,i) => {
            ...
            if(i == 0) cols.push({
                id : "P_"+(m_size-i), adjust : true,
                header : [
                    {text : "Mois de création", colspan : m_size+1, css:{ "text-align":"center" }},
                    {text : m, css:{ "text-align":"center" } }
                ],
                css: { "text-align":"center","font-family" : "Roboto, sans-serif", "font-size" : "12px"}
            })            
            else cols.push({
                id : "P_"+(m_size-i), adjust : true,
                header : ["", 
            ...
            css: { "text-align":"center","font-family" : "Roboto, sans-serif", "font-size" : "12px"},fillspace : 1
	...
	init(view) {
        ...
        webix.extend(view,webix.ProgressBar)
            view.showProgress();
            view.disable();
            components['bills'].push({cmp : "recouvr:vue2:decay", data : getBillsChartData('recouvDecTab').config.id})
            ...
            getBillsChartData("recouvDecTab").attachEvent("onAfterLoad", () => {
                if($$("top:menu").getSelectedId() == 'bills' && view._isDataLoaded ==1){
