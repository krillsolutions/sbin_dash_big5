...
export default class BillByAggByItem extends JetView{
	...
	config() {
		...
		tooltip : {
             ...
             title : [
                {
                        text : dash_titles['bill'].fact_by_agg? dash_titles['bill'].fact_by_agg : "",
                        top : 5,
                        left : '50%',
                        textAlign: 'center',
                        textStyle : {
        ...
        init(view) {
                ...
                view.showProgress();
                view.disable()
                view._expDataId = getBillsChartData('bill_agg_exp').config.id;
                view._eType = 'treemap';
                components['bills'].push( {cmp : 'bill:vue1:billag', data :getBillsChartData("bill_agg").config.id});
                components['bills'].push( {cmp : "bill:vue1:export:billag", data :getBillsChartData('bill_agg_exp').config.id });
                getBillsChartData("bill_agg").waitData.then((d) => {
			    view._isDataLoaded = 1;
                view.parse(getBillsChartData("bill_agg"));
                view.enable();
                view.hideProgress();
                ...
                view.data.attachEvent("onStoreLoad", function () {
                    updateChartReady('bill:vue1:billag');
                  });
        }
