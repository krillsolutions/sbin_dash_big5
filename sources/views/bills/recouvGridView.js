...
export default class RecouvGridView extends JetView {
    ...
    config(){
        ...
        let cells = [], type_fact = this._type_fact
        let mths = getMonthsFromDate(getDates()['d1'], recouvr_months_offset)  
        let nCol = 2, k=0, r = 0,p = 0
        mths.forEach(pp => {
            
            if(k != nCol) {  
                cells.push({view : "panel", x : nCol*k, y : nCol*r,dx : 2,dy : 2, body : new RecouvreDetView(this.app, "", p,type_fact) })              
                k++
            }
            else {
                k =0 ,r++
        ...
        return {
            view : "gridlayout", gridColumns : 4, gridRows : mths.length, cells : cells, id : "grid:"+type_fact,
